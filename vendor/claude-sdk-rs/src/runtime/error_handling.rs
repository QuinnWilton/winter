    ...
    pub fn to_debug_string(&self) -> String {
        ...
        if !self.error_chain.is_empty() {
            ...
            for (i, error) in self.error_chain.iter().enumerate() {
                let _ = writeln!(debug_str, "  {}: {}", i + 1, error);
            }
        }
    ...
    fn get_os_info() -> String {
        #[cfg(target_os = "macos")]
        {
            std::process::Command::new("sw_vers")
                ...
                .map_or_else(
                    || "macOS".to_string(),
                    |version| format!("macOS {}", version.trim()),
                )
        }
        #[cfg(target_os = "linux")]
        {
...
impl ProcessErrorDetails {
    ...
    pub fn new(message: impl Into<String>, command: impl Into<String>, args: Vec<String>) -> Self {
        Self {
            ...
            timestamp: std::time::SystemTime::now(),
            network_status: Some(Self::check_network_status()),
        }
    }
        ...
        relevant_vars
            ...
            .filter_map(|var| {
                std::env::var(var).ok().map(|value| {
                    ...
                    let is_sensitive =
                        var.contains("KEY") || var.contains("TOKEN") || var.contains("PASSWORD");
                    let is_long_path = *var == "PATH" && value.len() > 200;
    ...
    fn check_network_status() -> String {
        // Simple check - try to resolve a well-known domain
        match std::net::ToSocketAddrs::to_socket_addrs(&("anthropic.com", 443)) {
            Ok(_) => "Connected (DNS resolution successful)".to_string(),
            Err(e) => format!("Network issue: {e}"),
        }
    }
}
...
impl ErrorRecovery {
    ...
    pub fn recover_binary_not_found() -> Result<()> {
        ...
        for path in &common_paths {
            if std::path::Path::new(path).exists() {
                ...
                return Err(Error::ProcessError(format!(
                    "Claude CLI found at {path} but not in PATH. Please add it to your PATH environment variable"
                )));
            }
        }
...
After installation, ensure the binary is in your PATH.";
    ...
    pub fn recover_not_authenticated() -> Result<()> {
    ...
    pub fn recover_timeout(timeout_secs: u64) -> Result<()> {
        debug!(
...
4. Verifying Claude API service status",
    ...
    pub fn recover_rate_limit_exceeded() -> Result<()> {
...
4. Batch requests to reduce API calls",
    ...
    pub fn recover_mcp_error(error_message: &str) -> Result<()> {
...
4. Enable debug logging for protocol details"
            ...
            format!(
                r"MCP server error: {error_message}
...
For persistent issues, consult MCP documentation."
    ...
    pub fn recover_stream_closed() -> Result<()> {
...
```";
    ...
    fn get_retry_after_duration() -> Duration {
