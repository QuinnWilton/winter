...
use super::{
    store::EnhancedHistoryStore, ExportFormat, HistoryEntry, HistorySearch, HistoryStats,
    HistoryStore, SortField,
};
use crate::cli::{error::InteractiveError, session::SessionId};
    ...
    pub fn edge_case_entries(session_id: SessionId) -> Vec<HistoryEntry> {
        vec![
            ...
            HistoryEntry::new(
                ...
                vec![
                    "--path=/tmp/test$file&name".to_string(),
                    "--regex=\\d+\\s*".to_string(),
                ],
