...
//! It cannot be run as a public example since it uses internal types from winter_claude_core
//! that are not publicly exported.

fn main() {
    eprintln!("This example requires internal crate access and cannot be run as a public example.");
    eprintln!("It uses types from winter_claude_core that are not publicly exported.");
