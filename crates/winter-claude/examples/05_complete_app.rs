...
use winter_claude::{Client, Message, SessionId, StreamFormat, ToolPermission};
...
impl ClaudeAssistant {
    fn new(mode: AppMode) -> winter_claude::Result<Self> {
    ...
    async fn run(&mut self) -> winter_claude::Result<()> {
        self.print_welcome();
    ...
    async fn process_query(&mut self, query: &str) -> winter_claude::Result<()> {
        // Stream the response
        let mut stream = self
            .client
            .query(query)
...
fn create_chat_client() -> winter_claude::Result<Client> {
    Client::builder()
...
fn create_dev_client() -> winter_claude::Result<Client> {
    Client::builder()
        .stream_format(StreamFormat::StreamJson)
        .system_prompt(
...
fn create_analysis_client() -> winter_claude::Result<Client> {
    Client::builder()
        .stream_format(StreamFormat::StreamJson)
        .system_prompt(
...
#[tokio::main]
async fn main() -> winter_claude::Result<()> {
