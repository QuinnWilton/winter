...
///
/// ```rust
/// use winter_claude_core::{Error, Result};
///
...
impl Clone for Error {
    fn clone(&self) -> Self {
        match self {
            ...
            // Convert serialization errors to ProcessError to preserve the message.
            // serde_json::Error doesn't implement Clone, so we capture the description.
            Error::SerializationError(e) => {
                Error::ProcessError(format!("Serialization error: {e}"))
            }
            ...
            // FromUtf8Error contains the invalid bytes, so we recreate the error.
            Error::Utf8Error(e) => {
                Error::Utf8Error(String::from_utf8(e.as_bytes().to_vec()).unwrap_err())
            }
        }
    }
}
